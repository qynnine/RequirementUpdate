 ...
 */
public class VSM_Both implements IRModel {
    ...
    private SimilarityMatrix Compute(TermDocumentMatrix source, TermDocumentMatrix target) {
        ...
        TermDocumentMatrix TF_source = ComputeTF(source);
        double[] IDF_source = ComputeIDF(ComputeDF(source), source.NumDocs());
        TermDocumentMatrix TFIDF_source = ComputeTFIDF(TF_source, IDF_source);

        TermDocumentMatrix TF_target = ComputeTF(target);
        double[] IDF_target = ComputeIDF(ComputeDF(target), target.NumDocs());

        TermDocumentMatrix TFIDF_target = ComputeTFIDF(TF_target, IDF_target);

        return ComputeSimilarities(TFIDF_source, TFIDF_target);
    }
